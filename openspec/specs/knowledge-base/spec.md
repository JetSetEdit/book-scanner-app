...
The knowledge base is the set of pages that explain how Subtext works (transparency), policies (privacy, terms), press resources, and FAQ. It provides a single entry point (hub), consistent back-navigation to the hub from every child page, aligned labels between hub and child pages, and verification discipline so public claims stay accurate against the codebase. The project maintains `docs/KNOWLEDGE_BASE_VERIFICATION.md` to record which claims have been checked and any exceptions; that doc SHALL be updated when knowledge base copy or relevant code changes.
