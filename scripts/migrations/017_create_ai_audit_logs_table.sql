-- Create audit_logs table to track AI decision-making
CREATE TABLE IF NOT EXISTS public.ai_audit_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  book_id UUID NOT NULL REFERENCES public.books(id) ON DELETE CASCADE,
  scan_id UUID, -- Future: link to scans table when created
  isbn TEXT NOT NULL,
  ...
  -- AI Reasoning
  ai_reasoning TEXT NOT NULL, -- Why warnings were/weren't generated
  confidence_level TEXT CHECK (confidence_level IN ('low', 'medium', 'high')),
  
  -- Context
  book_title TEXT,
  book_author TEXT,
  description_length INTEGER, -- Length of description used
...
-- This is handled by using supabaseAdmin which bypasses RLS

COMMENT ON TABLE public.ai_audit_logs IS 'Audit trail of AI decisions for content warning generation';
COMMENT ON COLUMN public.ai_audit_logs.ai_reasoning IS 'Detailed explanation of why warnings were or were not generated';
COMMENT ON COLUMN public.ai_audit_logs.raw_ai_response IS 'Full AI response for debugging and transparency';
