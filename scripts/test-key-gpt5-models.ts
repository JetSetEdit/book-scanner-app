#!/usr/bin/env tsx
/**
 * Test key GPT-5 models (faster version)
 */
...
// Test only the most important models
const KEY_MODELS = [
  'gpt-5',              // Latest base
  'gpt-5.2',            // Latest version
  'gpt-5.2-pro',        // Most capable
  'gpt-5.2-pro-2025-12-11', // Specific version
  'gpt-5-mini',         // Fastest
  'gpt-5.1',            // Mid version
]
...
async function testModel(model: string): Promise<TestResult> {
  ...
  try {
    ...
    const result = await processIsbnScan(
      TEST_BOOK_ISBN,
      (msg) => {
        // Suppress most progress messages
...
async function main() {
  console.log('🚀 Testing Key GPT-5 Models')
  console.log('='.repeat(60))
  console.log(`📖 Test Book: Normal People (${TEST_BOOK_ISBN})`)
  console.log(`📊 Testing ${KEY_MODELS.length} key models\n`)
  ...
  console.log(`\n✅ Successful: ${successful.length}/${KEY_MODELS.length}`)
  console.log(`❌ Failed: ${failed.length}/${KEY_MODELS.length}`)
  
  if (successful.length > 0) {
    console.log('\n📈 Results by Model:')
    console.log('─'.repeat(60))
    console.log('Model'.padEnd(35) + 'Warnings'.padEnd(10) + 'Rating'.padEnd(10) + 'Latency')
    console.log('─'.repeat(60))
    ...
    successful.forEach(r => {
      ...
      console.log(`${r.model.padEnd(35)}${warnings}${rating}${latency}`)
    })
    ...
    // Consistency check
    const uniqueRatings = new Set(successful.map(r => r.rating))
    const uniqueWarningCounts = new Set(successful.map(r => r.warnings))
    
    console.log(`\n🎯 Consistency:`)
    console.log(`   Ratings: ${uniqueRatings.size} unique rating(s) - ${uniqueRatings.size === 1 ? '✅ Consistent' : '⚠️  Varies'}`)
    console.log(`   Warning Counts: ${uniqueWarningCounts.size} unique count(s) - ${uniqueWarningCounts.size === 1 ? '✅ Consistent' : '⚠️  Varies'}`)
  }
