...
import { z } from "zod";
import { contentWarningAgent, ContentWarning, ContentWarningSchema } from "./content-warning-agent";
import { bookInfoFinderAgent } from "./book-finder-agent";
...
`;
    ...
    try {
        const agentResult = await runner.run(
            contentWarningAgent, // Uses the imported agent
            conversationHistory
        );
